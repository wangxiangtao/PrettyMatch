...
final case class NGramLetterMetric(n: Int) extends StringMetric[Double] {
	def compare(a: Array[Char], b: Array[Char]): Option[Double] =
		if (n <= 0 || a.length < n || b.length < n) Some(0d) // Because length is less than n, it is not possible to compare.
